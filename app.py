...
# Add the summarize database function
def summarize_database():
    summary_prompt = """
    Please provide a concise summary of this database. Include:
    1. All tables in the database and their purpose
    2. Key columns in each table
    3. Important relationships between tables
    4. Approximate number of records in major tables (if available)
    Keep the summary brief but informative.
    """
    
    with st.spinner("Generating database summary..."):
        streamlit_callback = StreamlitCallbackHandler(st.container())
        summary = agent.run(summary_prompt, callbacks=[streamlit_callback])
    
    return summary

# Add Summarize Database button in the sidebar
